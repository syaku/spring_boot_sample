package net.sevenspirals.dbflute.bsbhv;
...
import net.sevenspirals.dbflute.bsbhv.loader.*;
import net.sevenspirals.dbflute.exentity.*;
import net.sevenspirals.dbflute.bsentity.dbmeta.*;
...
public abstract class BsUserBhv extends AbstractBehaviorWritable<User, UserCB> {
     ...
     * @throws SelectEntityConditionNotFoundException When the condition for selecting an entity is not found.
     */
    public OptionalEntity<User> selectByUniqueOf(String eMail) {
        return facadeSelectByUniqueOf(eMail);
    }

    protected OptionalEntity<User> facadeSelectByUniqueOf(String eMail) {
        return doSelectByUniqueOf(eMail, typeOfSelectedEntity());
    }

    protected <ENTITY extends User> OptionalEntity<ENTITY> doSelectByUniqueOf(String eMail, Class<? extends ENTITY> tp) {
        return createOptionalEntity(doSelectEntity(xprepareCBAsUniqueOf(eMail), tp), eMail);
    }

    protected UserCB xprepareCBAsUniqueOf(String eMail) {
     ...
     * <pre>
     * <span style="color: #0000C0">userBhv</span>.<span style="color: #CC4747">loadTweet</span>(<span style="color: #553000">userList</span>, <span style="color: #553000">tweetCB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     ...
     */
    public NestedReferrerListGateway<Tweet> loadTweet(List<User> userList, ReferrerConditionSetupper<TweetCB> refCBLambda) {
        xassLRArg(userList, refCBLambda);
        return doLoadTweet(userList, new LoadReferrerOption<TweetCB, Tweet>().xinit(refCBLambda));
    }
     ...
     * TWEET by USER_ID, named 'tweetList'.
     * <pre>
     * <span style="color: #0000C0">userBhv</span>.<span style="color: #CC4747">loadTweet</span>(<span style="color: #553000">user</span>, <span style="color: #553000">tweetCB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     <span style="color: #553000">tweetCB</span>.setupSelect...
     *     <span style="color: #553000">tweetCB</span>.query().set...
     *     <span style="color: #553000">tweetCB</span>.query().addOrderBy...
     * }); <span style="color: #3F7E5E">// you can load nested referrer from here</span>
     ...
     * <span style="color: #3F7E5E">//    ...</span>
     * <span style="color: #3F7E5E">//});</span>
     * ... = <span style="color: #553000">user</span>.<span style="color: #CC4747">getTweetList()</span>;
     * </pre>
     * About internal policy, the value of primary key (and others too) is treated as case-insensitive. <br>
     * The condition-bean, which the set-upper provides, has settings before callback as follows:
     * <pre>
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerListGateway<Tweet> loadTweet(User user, ReferrerConditionSetupper<TweetCB> refCBLambda) {
        xassLRArg(user, refCBLambda);
        return doLoadTweet(xnewLRLs(user), new LoadReferrerOption<TweetCB, Tweet>().xinit(refCBLambda));
    }

    protected NestedReferrerListGateway<Tweet> doLoadTweet(List<User> userList, LoadReferrerOption<TweetCB, Tweet> option) {
        return helpLoadReferrerInternally(userList, option, "tweetList");
    }
    ...
    { return helpExtractListInternally(userList, "EMail"); }
